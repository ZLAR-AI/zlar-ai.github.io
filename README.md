...
Unrouted shell/filesystem/browser/app/network/model-reasoning/final-text surfaces are not claimed as governed by the current proof path. `/contest` is not implemented. A private-by-default non-Vincent verifier request has been sent; no completed attestation has been received, and any result must be bounded by verifier relationship, disclosure permission, and exact evidence returned.
